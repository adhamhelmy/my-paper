...
\author{%
  ...
  Supervised by Prof.\ Mohamed Ashour \& Prof.\ Ayman El Badawy}
}
...
\begin{figure}[h]
  \centering
  \includegraphics[width=\columnwidth]{figures/RLHF.png}
...
\begin{figure}[h]
  \centering
  \includegraphics[width=\columnwidth]{figures/PPO.png}
...
\begin{table}[h]
  ...
  \begin{tabular}{clc}
    ...
    Rank & Outcome & Reward \\
    \midrule
    1 & No strategy generated                             & $-10$ (heavy penalty) \\
    2 & Syntax error                                      & $-3$  \\
    3 & Runtime error                                     & $-2$  \\
...
fine-tune the language model. Model was trained for 500 steps, with a learning rate of 5e-6, weight decay of 0.01, adam optimizer and gradient accumulation steps set to 8. Model was tested before and after training to evaluate the improvement in strategy generation performance.
...
\subsection{Dataset}
The proposed method was trained on historical stock prices of 38 US stocks, which are the constituents of the Dow Jones Industrial Average (DIJA) index, in addition to 8 additional stocks AMZN, COIN, GE, GOOGL, NFLX, NIO, TSLA, UVV, for the ten years from 2012-1-01 to 2022-12-31. The data was sourced from Alpha Vantage \cite{alphavantage2024}, a provider of free stock APIs. The data was preprocessed to ensure compatibility with the Backtrader framework.
And, tested against the same dataset, from 2022-1-01 to 2025-12-31, to evaluate model performance and compared against several baselines and benchmarks from the literature.
...
To contextualise the performance of our GRPO-trained model, we compare our model agains baseline, reinforcment learning-based, and LLM-based methods from the literature.
\begin{itemize}
  ...
  \item \textbf{Reinforcement-learning-based methods}
    \begin{itemize}
      ...
      \item \emph{A2C:} Advantage Actor-Critic, also trained within FinRL.
      \item \emph{DQN \cite{mnih2015human}:} Deep Q-Network adapted for discrete trading actions.
      \item \emph{SAC \cite{haarnoja2018soft}:} Soft Actor-Critic, an off-policy actor-critic
            algorithm that optimises trading strategies using entropy regularisation and soft
            value functions in continuous action spaces.
    \end{itemize}

  \item \textbf{LLM-based methods}
    \begin{itemize}
      \item \emph{INVESTORBENCH \cite{li2024investorbench}:} a benchmark for financial
            decision-making tasks with LLM-based agents.
